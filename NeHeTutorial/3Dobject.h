...
inline int ReadObject(const char *st, glObject *o){
  ...
  auto err = fopen_s(&file, st, "r");
  if ( err != 0 || !file) return FALSE;
  //points
  fscanf_s(file, "%d", &(o->nPoints));
  for (i=1;i<=o->nPoints;i++){
    fscanf_s(file, "%f", &(o->points[i].x));
    fscanf_s(file, "%f", &(o->points[i].y));
    fscanf_s(file, "%f", &(o->points[i].z));
  }
  //planes
  fscanf_s(file, "%d", &(o->nPlanes));
  for (i=0;i<o->nPlanes;i++){
    fscanf_s(file, "%d", &(o->planes[i].p[0]));
    fscanf_s(file, "%d", &(o->planes[i].p[1]));
    fscanf_s(file, "%d", &(o->planes[i].p[2]));

    fscanf_s(file, "%f", &(o->planes[i].normals[0].x));
    fscanf_s(file, "%f", &(o->planes[i].normals[0].y));
    fscanf_s(file, "%f", &(o->planes[i].normals[0].z));
    fscanf_s(file, "%f", &(o->planes[i].normals[1].x));
    fscanf_s(file, "%f", &(o->planes[i].normals[1].y));
    fscanf_s(file, "%f", &(o->planes[i].normals[1].z));
    fscanf_s(file, "%f", &(o->planes[i].normals[2].x));
    fscanf_s(file, "%f", &(o->planes[i].normals[2].y));
    fscanf_s(file, "%f", &(o->planes[i].normals[2].z));
  }
  return TRUE;
}
...
inline void DrawGLObject(const glObject& o){
	unsigned int i, j;
...
inline void  CastShadow(glObject *o, float *lp){
