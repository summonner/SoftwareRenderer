...
class TVector {
	...
	public:
		...
		enum class TStatus { INVALID, DEFAULT, UNIT };
	...
	public:
		...
		TVector() : _x(0.0), _y(0.0), _z(0.0), _Status(TStatus::INVALID) {}

		TVector(double x, double y, double z) : _x(x), _y(y), _z(z), _Status(TStatus::DEFAULT) {}
		...
		int isUnit() const { return _Status==TStatus::UNIT; }

		int isDefault() const { return _Status==TStatus::DEFAULT; }

		int isValid() const { return _Status!=TStatus::INVALID; }
		...
		TVector operator-() const { TVector r; return invert( *this, r ); }
