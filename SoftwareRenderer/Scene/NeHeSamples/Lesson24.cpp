...
namespace NeHe
{
	...
	bool Lesson24::LoadTGA( TextureImage* texture, const char* filename )			// Loads A TGA File Into Memory
	{
...
#pragma warning ( default:6385 6386 )
	...
	GLvoid Lesson24::BuildFont( GLvoid )									// Build Our Font Display List
	{
	...
	GLvoid Lesson24::KillFont( GLvoid )										// Delete The Font From Memory
	{
		glDeleteLists( base, 256 );								// Delete All 256 Display Lists
	}

	GLvoid Lesson24::glPrint( GLint x, GLint y, int set, const char* fmt, ... )	// Where The Printing Happens
	{
	...
	GLvoid Lesson24::ReSizeGLScene( GLsizei width, GLsizei height )			// Resize And Initialize The GL Window
	{
	...
	int Lesson24::InitGL( GLvoid )											// All Setup For OpenGL Goes Here
	{
	...
	int Lesson24::DrawGLScene( GLvoid )										// Here's Where We Do All The Drawing
	{
		...
		const char* source = (char*)glGetString( GL_EXTENSIONS );
		char* text = (char*)malloc( strlen( source ) + 1 );	// Allocate Memory For Our Extension String
		if ( text == nullptr )
		{
			return FALSE;
		}
		strcpy_s( text, strlen( source ) + 1, source );		// Grab The Extension List, Store In Text

		char* context = nullptr;
		token = strtok_s( text, " ", &context );									// Parse 'text' For Words, Seperated By " " (spaces)
		while ( token != NULL )										// While The Token Isn't NULL
		{
			...
			token = strtok_s( NULL, " ", &context );								// Search For The Next Token
		}
