...
namespace NeHe
{
	...
	bool Lesson32::LoadTGA( TextureImage* texture, const char* filename )				// Loads A TGA File Into Memory
	{
...
#pragma warning ( default : 6386 6385 )
	...
	GLvoid Lesson32::BuildFont( GLvoid )										// Build Our Font Display List
	{
	...
	GLvoid Lesson32::glPrint( GLint x, GLint y, const char* string, ... )		// Where The Printing Happens
	{
	...
	int Lesson32::Compare( struct objects* elem1, struct objects* elem2 )		// Compare Function *** MSDN CODE MODIFIED FOR THIS TUT ***
	{
	...
	GLvoid Lesson32::InitObject( int num )										// Initialize An Object
	{
	...
	GLvoid Lesson32::ReSizeGLScene( GLsizei width, GLsizei height )
	{
	...
	int Lesson32::InitGL( GLvoid )					// Any OpenGL Initialization Goes Here
	{
		...
		if ( (!LoadTGA( &textures[0], "Data/BlueFace.tga" )) ||			// Load The BlueFace Texture
			...
			(!LoadTGA( &textures[9], "Data/Font_32.tga" )) )				// Load The Crosshair Texture
		{
	...
	int Lesson32::CleanGL( GLvoid )										// Any User DeInitialization Goes Here
	{
		glDeleteLists( base, 95 );										// Delete All 95 Font Display Lists
		return TRUE;
	}

	void Lesson32::Selection( void )											// This Is Where Selection Is Done
	{
		...
		PlaySound( _T("data/shot.wav"), NULL, SND_ASYNC );					// Play Gun Shot Sound
	...
	void Lesson32::Update( DWORD milliseconds, bool keys[] )									// Perform Motion Updates Here
	{
		if ( keys[' '] && game )							// Space Bar Being Pressed After Game Has Ended?
		{
			for ( int loop = 0; loop < 30; loop++ )							// Loop Through 30 Objects
				InitObject( loop );										// Initialize Each Object
	...
	void Lesson32::Object( float width, float height, GLuint texid )				// Draw Object Using Requested Width, Height And Texture
	{
	...
	void Lesson32::Explosion( int num )											// Draws An Animated Explosion For Object "num"
	{
	...
	void Lesson32::DrawTargets( void )											// Draws The Targets (Needs To Be Seperate)
	{
		glLoadIdentity();											// Reset The Modelview Matrix
	...
	int Lesson32::DrawGLScene( GLvoid )													// Draw Our Scene
	{
		...
		// Crosshair (In Ortho View)												// Storage For Window Dimensions
		glMatrixMode( GL_PROJECTION );								// Select The Projection Matrix
		glPushMatrix();												// Store The Projection Matrix
		glLoadIdentity();											// Reset The Projection Matrix
		glOrtho( 0, window.right, 0, window.bottom, -1, 1 );				// Set Up An Ortho Screen
		glMatrixMode( GL_MODELVIEW );									// Select The Modelview Matrix
		glTranslated( mouse_x, (GLdouble)window.bottom - mouse_y, 0.0f );			// Move To The Current Mouse Position
