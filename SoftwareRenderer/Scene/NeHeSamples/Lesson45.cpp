...
namespace NeHe
{
	...
	public:
		...
		// Heightmap Loader
		bool LoadHeightmap( LPCTSTR szPath, float flHeightScale, float flResolution );
		// Single Point Height
	...
	BOOL LESSON::InitGL()					// Any GL Init Code & User Initialiazation Goes Here
	{
		...
		if ( !g_pMesh->LoadHeightmap( _T( "Data/terrain.bmp" ),					// Load Our Heightmap
			MESH_HEIGHTSCALE,
			MESH_RESOLUTION ) )
		{
			MessageBox( NULL, _T( "Error Loading Heightmap" ), _T( "Error" ), MB_OK );
			return false;
		}
	...
	GLvoid LESSON::ReSizeGLScene( GLsizei width, GLsizei height )			// Reshape The Window When It's Moved Or Resized
	{
	...
	bool CMesh::LoadHeightmap( LPCTSTR szPath, float flHeightScale, float flResolution )
	{
		// Error-Checking
